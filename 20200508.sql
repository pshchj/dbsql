...
2. ON DELETE CASCADE ==>  부모가 삭제되면 참조하고 잇는 자식 데이터도 같이 삭제
...
2. 테이블 명세서가 정확ㄷ하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
   (java코드 + 테이블 내역 확인 필요)
...
이미 생성된 테이블을 수정 할 수도 있음
    1. 새로운 컬럼을 추가
       ...
       ****** 기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가할 수가 업삳
    ...
    2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경, (타입도 제한적으로 변경 가능)
       참조키가 걸려잇는 테이블의 컬럼 이름을 변경하더라도 참조하는 테이블에는 영향이 가지 않음
       (알아서 이름을 변경 해준다)
